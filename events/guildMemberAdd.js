...
member.setNickname(`${tag} ${member.user.username}`)
  ...
  member.addRole(rol)
    ...
    if (db.has(`otoRK_${member.guild.id}`) === true) {
      if(kanalD === "var"){
    member.guild.channels.get(db.fetch(`otoRK_${member.guild.id}`)).send(`:inbox_tray: Otomatik Rol Verildi Seninle Beraber **${member.guild.members.size}** Kişiyiz! ${client.emojis.get(client.emojiler.mutlu)} Aramıza Hoşgeldin **${member.user.tag}**`)
  }}}};
